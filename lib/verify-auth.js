...
module.exports = async (npmrc, pkg, context) => {
  ...
  if (!process.env.CFA_SECRET) {
    throw new AggregateError([getError('EINVALIDCFASECRET')]);
  }

  if (!process.env.CFA_PROJECT_ID) {
    throw new AggregateError([getError('EINVALIDCFAPROJECTID')]);
  }
};
